<?php
   /**
   * Блок произвольного опроса
   * 
   * @author Igor Ognichenko
   * @copyright Copyright (c)2007-2010 by Kasseler CMS
   * @link http://www.kasseler-cms.net/
   * @filesource blocks/block-voting_random.php
   * @version 2.0
   */
   ...
   $result = $main->db->sql_query("SELECT 
    ...
    FROM ".VOTING." AS v WHERE ((module is null) or module='') AND status='1' ORDER BY RAND() LIMIT 1");
   if($main->db->sql_numrows($result)>0){
      ...
      if($acc_ok AND empty($vote['found_vote']) AND mb_strpos($cook, $vote['id'].",")===false){
         ...
         echo "<h2 class='vote_title'>{$vote['title']}</h2><table width='100%'><tr><td>";
         $pl=1;
         for($y=0; $y<count($tmpcase)-1; $y++){
            $case = $tmpcase[$y];
            $res = 50;
            ...
            $proc = round( $res, 2 );
            echo "<div class='vote'>{$case}  - {$vote[$case]} ({$proc}%)</div><div class='progress voted progress_{$pl}'><span style='width: ".intval($proc)."%;'>&nbsp;</span></div>";
            $pl++;
            if($pl == 6) $pl=1;
         }
         echo "<td></tr><tr><td align='center'><br />{$main->lang['votes']}: (".$sum.")<br /><a class='other_votes link_button' href='".$main->url(array('module' => 'voting'))."' title='{$main->lang['other_votes']}'>{$main->lang['other_votes']}</a></td></tr></table>";
      }
   } else echo "&nbsp;";
?>
