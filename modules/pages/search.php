...
* @filesource modules/pages/search.php
* @version 2.0
*/
    ...
    foreach($strings as $value){
        if(mb_strlen($value)>=$length_ignore){
            switch($_S['search_type']){
                ...
                case "1": $where_words .= " OR UPPER(begin) LIKE '%".mb_strtoupper($value)."%' OR UPPER(content) LIKE '%".mb_strtoupper($value)."%'"; break;
                case "2": $where_words .= " OR UPPER(title) LIKE '%".mb_strtoupper($value)."%' OR UPPER(begin) LIKE '%".mb_strtoupper($value)."%' OR UPPER(content) LIKE '%".mb_strtoupper($value)."%'"; break;
            }
        }
    }
            ...
            foreach($strings as $value){
                if(mb_strlen($value)>=$length_ignore) {
                    switch($_S['search_type']){
                        ...
                        case "1": $keywords .= mb_strpos($row['begin'], $value) !== false ? $value." " : ''; break;
                        case "2": 
                            $keywords .= mb_strpos($row['title'], $value) !== false ? $value." " : '';
                            $keywords .= mb_strpos($row['begin'], $value) !== false ? $value." " : '';
                            break;
                    }                    
                }
            }
        }
        $insert .= "(NULL, '{$search_key}', '".addslashes($row['title'])."', '{$row['author']}', '".addslashes($row['begin'])."', '{$row['date']}', 'pages', '{$row['id']}', '{$row['pages_id']}', '".time()."', '{$keywords}'),";
    }        
