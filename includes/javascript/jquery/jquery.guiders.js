...
var guiders = (function($) {
  ...
  guiders._addButtons = function(myGuider) {
    ...
    for (var i = myGuider.buttons.length-1; i >= 0; i--) {
      ...
      var thisButtonElem = $("<a></a>", {
                              "class" : "guider_button styles_button button",
  ...
  guiders._attach = function(myGuider) {
    ...
    if (myGuider.position === 0 || myGuider.attachTo === null) {
      ...
      if(myGuider.offset.top===null) myGuider.elem.css({"top": ($(window).height() - myHeight) / 3 + $(window).scrollTop() + "px", "left": ($(window).width() - myWidth) / 2 + $(window).scrollLeft() + "px"});
      else myGuider.elem.css({"top": myGuider.offset.top, "left": myGuider.offset.left});
      return;
    }
