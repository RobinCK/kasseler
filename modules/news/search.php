...
* @filesource modules/news/search.php
* @version 2.0
*/
...
if($main->db->sql_numrows($result)>0){
    ...
    while($row = $main->db->sql_fetchrow($result)){
        ...
        $insert .= "(NULL, '{$search_key}', '".addslashes($row['title'])."', '{$row['author']}', '".addslashes($row['begin'])."', '{$row['date']}', 'news', '{$row['id']}', '{$row['news_id']}', '".time()."', '{$keywords}'),";
    }
